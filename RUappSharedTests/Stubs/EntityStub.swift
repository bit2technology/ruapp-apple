...
class EntityStub {
  ...
  static func institution(context: NSManagedObjectContext) -> Institution {
    let institution = Institution.new(in: context)
    institution.id = 1
  ...
  static func campus(context: NSManagedObjectContext) -> Campus {
    let campus = Campus.new(in: context)
    campus.id = 1
    campus.name = "Stub"
  ...
  static func cafeteria(context: NSManagedObjectContext) -> Cafeteria {
    let cafeteria = Cafeteria.new(in: context)
    cafeteria.id = 1
...
extension NamedManagedObject {
  static func new(in context: NSManagedObjectContext) -> Self {
    return self.init(entity: Self.entity(in: context), insertInto: context)
  }
}
